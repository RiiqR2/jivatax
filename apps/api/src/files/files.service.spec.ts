...
interface Harness {
  ...
  repository: {
    ...
    findOneBy: (
      where: Partial<StoredFileEntity>,
    ) => Promise<StoredFileEntity | null>;
  };
...
function harness(
  ...
  const repository = {
    ...
    create: (data: Partial<StoredFileEntity>) =>
      Object.assign(new StoredFileEntity(), data),
  ...
  const companies = {
    companiesRepository: {
      existsBy: async () => options.companyExists ?? true,
    },
...
describe("FilesService", () => {
  it("creates a company-scoped signed upload URL without exposing the bucket", async () => {
    ...
    assert.match(
      result.objectKey,
      new RegExp(`^companies/${companyId}/input/.+\\.xlsx$`),
    );
  ...
  it("rejects files over the configured maximum size", async () => {
    ...
    await assert.rejects(
      () => service.createUploadUrl(companyId, metadata),
      PayloadTooLargeException,
    );
  });
  ...
  it("registers only trusted input/uploaded metadata after checking storage", async () => {
    ...
    const result = await service.completeUpload(companyId, {
      ...metadata,
      objectKey,
    });
  ...
  it("rejects an object key already registered", async () => {
    ...
    await assert.rejects(
      () =>
        service.completeUpload(companyId, {
          ...metadata,
          objectKey: `companies/${companyId}/input/upload.xlsx`,
        }),
      ConflictException,
    );
  });
  ...
  it("downloads only a file selected by company and suggests its original name", async () => {
    ...
    saved.push(
      Object.assign(new StoredFileEntity(), {
        id: "file-id",
        companyId,
