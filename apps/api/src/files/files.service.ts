...
export class FilesService {
  ...
  async completeUpload(
    ...
    const bucket = this.getBucket();
    if (
      await this.storedFilesRepository.existsBy({
        bucket,
    ...
    if (!(await this.storage.objectExists(bucket, dto.objectKey))) {
      throw new BadRequestException(
        "The uploaded object does not exist in storage",
      );
    }
  ...
  async createDownloadUrl(
    ...
    await this.assertCompanyAccess(companyId);
    const file = await this.storedFilesRepository.findOneBy({
      id: fileId,
      companyId,
    });
  ...
  private async assertCompanyAccess(companyId: string): Promise<void> {
    const companyExists =
      await this.companiesService.companiesRepository.existsBy({
        id: companyId,
      });
  ...
  private validateMetadata(dto: FileMetadataDto): void {
    ...
    try {
      ...
      throw new BadRequestException(
        "sizeBytes must be a non-negative integer string",
      );
    }
    if (size < 0n)
  ...
  private getExtension(originalName: string): string {
    ...
    if (extension.length > 20) {
      throw new BadRequestException(
        "File extension must not exceed 20 characters",
      );
    }
    return extension;
  }
  ...
  private getMaxFileSize(): bigint {
    const configured = this.config.getOrThrow<string>(
