...
@Index('uq_organization_members_organization_id_user_id', ['organizationId', 'userId'], {
  unique: true,
})
...
export class OrganizationMemberEntity extends AuditableEntity {
  ...
  @Column({ type: 'enum', enum: OrganizationMemberStatus, default: OrganizationMemberStatus.INVITED })
  ...
  @JoinColumn({ name: 'organization_id', foreignKeyConstraintName: 'fk_organization_members_organization_id' })
  organization!: OrganizationEntity;

  @ManyToOne(() => UserEntity, (user) => user.organizationMembers, { onDelete: 'RESTRICT' })
  @JoinColumn({ name: 'user_id', foreignKeyConstraintName: 'fk_organization_members_user_id' })
  user!: UserEntity;
}
