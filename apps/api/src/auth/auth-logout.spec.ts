...
function harness(cookies: Record<string, string>) {
  ...
  const jwt = { verifyAsync: async (token: string, options: { secret: string; ignoreExpiration?: boolean }) => {
    assert.equal(options.ignoreExpiration, true);
    if (token === 'expired-access' && options.secret === accessSecret) return { sessionId: 'session-1' };
    if (token === 'valid-refresh' && options.secret === refreshSecret) return { sessionId: 'session-1' };
    throw new Error('invalid token');
  } } as unknown as JwtService;
  const sessions = { update: async (criteria: unknown, values: unknown) => { updates.push({ criteria, values }); } } as unknown as Repository<AuthSessionEntity>;
  ...
  const response = { clearCookie: (name: string, options: unknown) => { cleared.push({ name, options }); } } as unknown as Response;
...
describe('AuthService logout', () => {
  it('revokes the session identified by an expired access token and clears both cookies', async () => {
    ...
    const criteria = test.updates[0].criteria as { id: string; revokedAt: { _type: string } };
    assert.equal(criteria.id, 'session-1');
    assert.equal(criteria.revokedAt._type, 'isNull');
    assert.ok((test.updates[0].values as { revokedAt: Date }).revokedAt instanceof Date);
    assert.deepEqual(test.cleared.map(({ name }) => name), ['jivatax.access', 'jivatax.refresh']);
  });
