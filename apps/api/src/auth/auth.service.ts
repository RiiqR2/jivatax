...
export type SessionResponse = {
  user: Omit<
    AuthenticatedUser,
    "sessionId" | "currentOrganizationId" | "status"
  >;
...
export class AuthService {
  ...
  constructor(
    ...
    this.accessTtl = Number(
      config.get("JWT_ACCESS_TTL_SECONDS", DEFAULT_ACCESS_TTL_SECONDS),
    );
    this.refreshTtl = Number(
      config.get("JWT_REFRESH_TTL_SECONDS", DEFAULT_REFRESH_TTL_SECONDS),
    );
  }
  ...
  async validateCredentials(
    ...
    if (!verified) {
      if (user && user.status === UserStatus.ACTIVE) {
        ...
        await this.users.usersRepository.save(user);
      }
  ...
  async login(
  ...
  ): Promise<{ body: SessionResponse; access: string; refresh: string }> {
    ...
    const tokens = await this.issue(user.id, session.id);
    session.refreshTokenHash = await argon2.hash(tokens.refresh, {
  ...
  async me(user: AuthenticatedUser): Promise<SessionResponse> {
    const organizations = await this.organizations(user.id);
    const current = organizations.some(
      (item) => item.id === user.currentOrganizationId,
    )
  ...
  async refresh(
    ...
    if (!previous || previous.userId !== payload.sub) {
      throw new UnauthorizedException();
    }
    ...
    if (
      ...
      previous.user.status !== UserStatus.ACTIVE ||
      previous.user.deletedAt ||
      !(await argon2
        .verify(previous.refreshTokenHash, token)
        .catch(() => false))
    )
      throw new UnauthorizedException();
    const next = await this.sessions.save(
      this.sessions.create({
        userId: previous.userId,
        refreshTokenHash: "pending",
        currentOrganizationId: previous.currentOrganizationId,
        expiresAt: new Date(Date.now() + this.refreshTtl * 1000),
        revokedAt: null,
        replacedBySessionId: null,
        lastUsedAt: null,
    ...
    const tokens = await this.issue(previous.userId, next.id);
    next.refreshTokenHash = await argon2.hash(tokens.refresh, {
      type: argon2.argon2id,
    });
    await this.sessions.save(next);
    previous.revokedAt = new Date();
    previous.lastUsedAt = new Date();
  ...
  ): Promise<{ body: SessionResponse; access: string }> {
    const organizations = await this.organizations(user.id);
    if (!organizations.some((item) => item.id === organizationId)) {
      throw new ForbiddenException(
        "No pertenece a la organización seleccionada.",
      );
    }
    await this.sessions.update(user.sessionId, {
      currentOrganizationId: organizationId,
    });
    return {
      body: this.response(user, user.sessionId, organizationId, organizations),
      access: await this.signAccess(user.id, user.sessionId),
    };
  }
  ...
  private async sessionIdFromAnyToken(
    access?: string,
    refresh?: string,
  ...
  private async organizations(userId: string): Promise<OrganizationSummary[]> {
  ...
  private response(
    user: Pick<
      UserEntity,
      "id" | "email" | "firstName" | "lastName" | "platformRole"
    >,
    _sessionId: string,
    ...
    return {
      ...
      organization: organizations.find((item) => item.id === currentId) ?? null,
      organizations,
      requiresOrganizationSelection: organizations.length > 1 && !currentId,
    };
  }
  private async issue(
    userId: string,
    ...
    return {
      access: await this.signAccess(userId, sessionId),
      refresh: await this.jwt.signAsync<RefreshTokenPayload>(
        { sub: userId, type: REFRESH_TOKEN_TYPE, sessionId, jti: randomUUID() },
        { secret: this.refreshSecret, expiresIn: this.refreshTtl },
      ),
    };
  }
  private signAccess(userId: string, sessionId: string): Promise<string> {
    return this.jwt.signAsync<AccessTokenPayload>(
      { sub: userId, type: ACCESS_TOKEN_TYPE, sessionId },
