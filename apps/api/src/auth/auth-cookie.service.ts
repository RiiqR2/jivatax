...
export class AuthCookieService {
  ...
  get accessName(): string {
    return this.config.get("ACCESS_COOKIE_NAME", "jivatax.access");
  }
  get refreshName(): string {
    return this.config.get("REFRESH_COOKIE_NAME", "jivatax.refresh");
  }
  ...
  clearRefreshCookie(res: Response): void {
    res.clearCookie(
      this.refreshName,
      this.withoutMaxAge(this.refreshOptions()),
    );
  }
  ...
  private accessOptions(): CookieOptions {
    return this.options(
      "/",
      Number(
        this.config.get("JWT_ACCESS_TTL_SECONDS", DEFAULT_ACCESS_TTL_SECONDS),
      ) * 1000,
    );
  }
  ...
  private refreshOptions(): CookieOptions {
    return this.options(
      "/api/auth",
      Number(
        this.config.get("JWT_REFRESH_TTL_SECONDS", DEFAULT_REFRESH_TTL_SECONDS),
      ) * 1000,
    );
  }
