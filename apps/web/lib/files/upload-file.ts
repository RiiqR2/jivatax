...
export async function uploadCompanyFile(
  ...
  const { uploadUrl, objectKey } = await api.createUploadUrl(companyId, metadata);
...
export function validateUpload(file: File | null, category: FileCategory | '', maxSizeBytes: number) {
  const errors: { file?: string; category?: string } = {};
  if (!file) errors.file = 'Selecciona un archivo.';
  else if (file.size > maxSizeBytes) errors.file = 'El archivo supera el tamaño máximo permitido.';
