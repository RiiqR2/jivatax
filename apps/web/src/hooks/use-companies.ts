...
export function useCreateCompany() {
  ...
  return useMutation({
    ...
    onSuccess: () =>
      client.invalidateQueries({ queryKey: companyKeys.lists() }),
  });
}
export function useUpdateCompany(id: string) {
  const client = useQueryClient();
  return useMutation({
    mutationFn: (input: CompanyInput) => companiesService.update(id, input),
    onSuccess: async () => {
      await Promise.all([
        client.invalidateQueries({ queryKey: companyKeys.lists() }),
        client.invalidateQueries({ queryKey: companyKeys.detail(id) }),
      ]);
    },
  });
}
