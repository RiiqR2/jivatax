...
const companyFile: CompanyFile = {
  id: 'file-1', originalName: 'balance-2025.xlsx', extension: 'xlsx',
  contentType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
  sizeBytes: '1048576', category: 'balance', direction: 'input', status: 'uploaded',
...
test('valida archivo y categoría obligatorios y el tamaño máximo', () => {
  assert.deepEqual(validateUpload(null, '', 10), {
    file: 'Selecciona un archivo.', category: 'Selecciona una categoría.',
  });
...
test('ejecuta upload-url, PUT y complete en orden', async () => {
  const calls: string[] = [];
  const api = {
    createUploadUrl: async () => { calls.push('upload-url'); return { uploadUrl: 'https://storage.test/upload', objectKey: 'private-key', expiresIn: 900 }; },
    uploadToStorage: async () => { calls.push('PUT'); },
    complete: async (_companyId: string, payload: { objectKey: string }) => { calls.push(`complete:${payload.objectKey}`); return companyFile; },
  } as FilesApi;
  await uploadCompanyFile(api, 'company-1', new File(['ok'], 'balance.xlsx', { type: 'application/xlsx' }), 'balance', { onStage: () => {}, onProgress: () => {} });
...
test('detiene el flujo cuando falla la subida al storage', async () => {
  let completed = false;
  const api = {
    createUploadUrl: async () => ({ uploadUrl: 'https://storage.test/upload', objectKey: 'key', expiresIn: 900 }),
    uploadToStorage: async () => { throw new Error('storage failed'); },
    complete: async () => { completed = true; return companyFile; },
  } as FilesApi;
  await assert.rejects(uploadCompanyFile(api, 'company-1', new File(['x'], 'file.xml'), 'xml', { onStage: () => {}, onProgress: () => {} }));
  assert.equal(completed, false);
});

test('solicita la URL de descarga con empresa y archivo', async () => {
  const requested: string[] = [];
  const api = { downloadUrl: async (companyId: string, fileId: string) => { requested.push(companyId, fileId); return { downloadUrl: 'https://storage.test/download', expiresIn: 900 }; } } as FilesApi;
