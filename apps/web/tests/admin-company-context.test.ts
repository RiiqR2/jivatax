...
test("administration keeps the active company in memory", async () => {
  ...
  assert.match(provider, /if \(companyId\)[\s\S]*setActiveCompanyId\(companyId\)/);
  assert.match(provider, /const resolvedCompanyId = companyId \?\? activeCompanyId/);
});
...
test("the admin header returns to the active company or company selection", async () => {
  ...
  assert.match(header, /`\/companies\/\$\{activeCompany\.id\}\/dashboard`/);
  assert.match(header, /: "\/companies"/);
});
